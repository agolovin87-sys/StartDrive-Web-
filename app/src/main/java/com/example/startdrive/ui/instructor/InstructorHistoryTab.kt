...
@Composable
fun InstructorHistoryTab(instructorId: String) {
    val drivingRepo = remember { DrivingRepository() }
    val userRepo = remember { UserRepository() }
    val sessions by drivingRepo.drivingHistory(instructorId, true).collectAsState(initial = emptyList())
    val balanceHistory by drivingRepo.balanceHistory(instructorId).collectAsState(initial = emptyList())
    ...
    var cadetNamesMap by remember { mutableStateOf<Map<String, String>>(emptyMap()) }
    ...
    LaunchedEffect(sessions) {
        ...
        cadetNamesMap = cadetIds.associateWith { id -> userRepo.getUser(id)?.fullName?.takeIf { it.isNotBlank() }?.let(::formatSurnameWithInitials) ?: "—" }
    }
                            ...
                            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                                completedSessions.forEach { s ->
                                    CompletedSessionCard(
                                        session = s,
                                        cadetName = cadetNamesMap[s.cadetId] ?: "—",
...
private fun CompletedSessionCard(
    ...
    val completedDateTime = session.completedAt?.toDate()?.let { dateTimeFormat.format(it) } ?: "—"
    val timerRemaining = session.completedTimerRemaining ?: ""
    Card(
