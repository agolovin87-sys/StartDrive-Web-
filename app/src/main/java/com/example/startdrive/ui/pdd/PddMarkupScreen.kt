...
fun PddMarkupScreen(
    ...
    var sections by remember { mutableStateOf<List<PddRepository.MarkupSection>>(emptyList()) }
    var selectedSection by remember { mutableStateOf<PddRepository.MarkupSection?>(null) }
    var selectedItem by remember { mutableStateOf<PddRepository.PddMarkupItem?>(null) }

    LaunchedEffect(Unit) {
        sections = PddRepository.loadMarkup(context)
    }
    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        when {
            selectedItem != null -> PddMarkupDetailContent(item = selectedItem!!)
            selectedSection != null -> {
                ...
                if (section != null) {
                    ...
                    LazyColumn(
                        ...
                        items(
                            sectionItems,
                            key = { it.number.ifEmpty { it.hashCode().toString() } },
                        ) { item ->
                            Card(
...
@Composable
private fun PddMarkupDetailContent(item: PddRepository.PddMarkupItem) {
    Column(
        modifier = Modifier
            .fillMaxSize()
